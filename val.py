...
if __name__ == '__main__':
    model_path = ('/root/autodl-tmp/code/rtdetr-r18/train/exp_rtdetr-r18/weights/best.pt')
    model = RTDETR(model_path)  # 选择训练好的权重路径

    result = model.val(data='/root/autodl-tmp/data/data_xin.yaml',
                       split='test',  #视情况选择val、test
                       imgsz=640,
                       batch=16,

                       project='rtdetr-r18/test',
                       name='exp'
                       )

    if model.task == 'detect':  # 仅目标检测任务适用python
